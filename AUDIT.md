...
- **2026-07-16 (cont'd — shipped):** Wave 1 of the roadmap above (call reliability, P0-first) landed on `claude/comms-features-architecture-gqtb1y`, plus two owner-requested UX items and an APK-distribution fix surfaced by real sideload testing. **Calls (Wave 1):** WP2 registered the missing Tauri commands `call_ice_servers_for`/`call_sas`/`turn_server_status`; WP1 fixed the confirmed cross-platform P0 — desktop `handleIncomingOffer` now treats a same-`call_id` re-offer as renegotiation, not `busy`, via a new pure `desktop/ui/call_decisions.mjs` + a `node --test` CI lane (`desktop-js`); WP3 gave the desktop caller STUN-first→TURN ICE-restart fallback (mirroring Android's `tryTurnFallbackOrFail`) with a 30s connect-timeout backstop, and fixed two latent desktop defects found in review (a second answer after an ICE-restart re-offer used to throw and tear down the call; the callee's own pre-connect failure used to pre-empt the caller's rescue); WP4 added the desktop SAS encryption-emoji UI; WP5 pinned the same-`call_id` re-offer + glare *transport* guarantees in `two_peer_integration.rs`. README/Kotlin doc-accuracy pass corrected the stale "Android calls are a follow-up"/"incoming persistence planned"/"no call screen" claims. **UX:** a Telegram-style `ModalNavigationDrawer` moved Call history + Settings off the bottom bar (now Chats/Journal/Feed) into a drawer with a profile header, and all passcode fields (create/confirm/unlock, one shared composable) switched to a numeric keypad with a digits-only create-time rule. **Distribution:** "App not installed" on sideloaded updates was root-caused to per-runner throwaway debug signatures — fixed by committing a stable `android/debug.keystore` (standard public debug credentials, never for Play) wired into the debug `signingConfig`, plus a cross-workflow monotonic `versionCode` = `git rev-list --count HEAD` (127, above every historical value) so debug and release artifacts order correctly. **Android device-lane stabilization:** the recurring `MainActivityUiTest` `RootViewWithoutFocusException` was traced to a *process crash* in the call tests killing the shared emulator process — two foreground-service contract violations: `CallService` never calling `startForeground` on a stop-before-start race / blank restart intent (`ForegroundServiceDidNotStartInTimeException`, fixed by an `onCreate` placeholder + `foregroundStarted` guard), and `CallManagerLifecycleTest` promoting a mic-typed FGS without the `RECORD_AUDIO` runtime grant API 34+ requires (`SecurityException`, fixed by granting it — matching production's `withCallPermissions` gate and the new `CallServiceContractTest`, which also carries two deterministic regression tests for the timing crash). **Verification honesty:** the Rust workspace + the new `node --test` lane are green locally; every Android and `desktop/src-tauri` change is uncompiled in this environment (no Android SDK; no GTK/webkit for Tauri) and rests on read-through + structural conformance — the `ci.yml` android/desktop lanes and the `android-apk.yml` emulator lanes are the real gate, and the device-lane crash fixes are **pending confirmation on the next `android-apk` run** at the time of writing. **Follow-ups surfaced:** (1) identity export/import (nsec) — the existing §7 gap, elevated to near-term after a real reinstall wiped a test identity with no recovery path; (2) `CallService` should defensively pick its FGS type from *granted* permissions to survive a mid-call permission revocation (deferred — the API-34 type-`NONE` behavior is unverifiable without a device); (3) **OQ10** (TURN operator + populating the empty CI `TURN_*` secrets) still gates the app's "No relay configured" state and any real NAT-traversal test.
...
| A3 | ~~H~~ | **[fact, adversarially confirmed, resolved 2026-07-16]** Couple pairing was foreclosed by the ownership design, not just unwired — `pair_with` took `&mut self` with no interior mutability while `ComradeRuntime` Arc-wrapped the engine, and the desktop "Partner Portal" validated the pairing token client-side with no key agreement. `SakhaEngine::pair_with` now takes `&self` and performs a real ECDH handshake (`compute_dh_shared_secret` + `derive_symmetric_key`) into `SyncRwLock`-guarded fields, reachable through the shared `Arc<SakhaEngine>` handle exactly as `ComradeRuntime` holds it (`crates/comrade_core/src/sakha.rs:163-172`), pinned by a dedicated regression test that names this finding (`pair_with_works_through_a_shared_arc_handle`, `sakha.rs:409-420`). The bridge is wired end-to-end: desktop's `handlePair` calls `safeInvoke("pair_sakha", …)` (`desktop/ui/main.js:1897-1909`) → the registered Tauri command (`desktop/src-tauri/src/commands.rs:120-128`, registered `lib.rs:97`) → `ComradeRuntime::pair_sakha` (`crates/comrade_ui/src/runtime.rs:1907-1932`), which calls `sakha.pair_with(peer)` and persists the pairing record; `subscribe_sync` now has a caller (`spawn_sakha_sync_loop`, `runtime.rs:1896`). The DM composer's Send button is likewise real, not a stub (`handleDmSend` invokes `send_dm`/`send_dm_reply`, `main.js:533-554`). |
...
| O5 | ~~L~~ | **[fact, resolved 2026-07-16]** The documented desktop build could not complete — `tauri.conf.json`'s `bundle.icon` list (`tauri.conf.json:26-31`) referenced files that didn't exist; `desktop/src-tauri/icons/` held only a README. The directory now contains real, non-empty icon assets matching every referenced path: `32x32.png` (32×32 PNG), `128x128.png` (128×128 PNG), `icon.png` (512×512 PNG), and `icon.ico` (256×256 Windows icon resource) — confirmed valid image binaries via `file`, not placeholders; `cargo tauri build` no longer fails on missing icons. Residual minor gap: the README quick-start (`README.md:93-97`) still doesn't document icon generation. |
