...
| **Tara** | On-device reply engine (no network, no cloud) | Reflective companion — a private space to think out loud: feeling-mirroring, reflective prompts, brainstorming scaffolds, journaling nudges from mood markers. **Not therapy, and it says so**: distress cues switch the reply into a hand-off to real crisis helplines. Thread sealed in the encrypted store; *clear conversation* built in | ⚠️ v1 deterministic template engine wired end-to-end (Android **Tara** tab with opt-in explainer + crisis card; desktop backend commands, web UI pending). The engine seam is ready for an **on-device** LLM once [`AUDIT.md` OQ9](AUDIT.md) is decided — a cloud model is ruled out by design. See [`docs/TARA.md`](docs/TARA.md) |
